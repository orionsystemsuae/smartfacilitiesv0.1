(function () {
    ...
    function Controller($scope, $rootScope, $state, $stateParams, $moment, $window, $interval, UserService, DashboardService, MetricsService, FlashService, WidgetFactory) {
        ...
        $scope.UpdateChartData = function (widget) {
            ...
            let widgetTimeStampMini = $scope.GetWidgetSetting(widget, 'bodyChartTimeStampMini');
            ...
            //assign x-Axis timestamps labels array
            if (widget.config.chart.series.xValues.length < timestampArray.length) {
                ...
                for (var i = 0; i < valueLimit && i < widget.config.chart.series.xValues.length; i++) {
                    if (widgetTimeStampMini)
                        widget.config.chart.series.xValues[i] = moment.utc(timestampArray[i]).format(timeStampFormat);
                    else
                        widget.config.chart.series.xValues[i] = timestampArray[i];
                }
            }
                                        ...
                                        if (result.results[0].series.length > 0) {
                                            ...
                                            Object.keys(topic.metrics).forEach(metric => {
                                                ...
                                                if (topic.metrics[metric] == true) {
                                                    ...
                                                    // switch (frequency) {
                                                    //     case '1m': for (var i = 0; i < valueLimit; i++) {
                                                    //         //check if value for this time stamp exists
                                                    ...
                                                    //     case '1h': for (var i = 0; i <= valueLimit; i = i + 60) {
                                                    ...
                                                    // console.log('timeStart', timeStart);
                                                    //console.log('yValuesArrayLength', widget.config.chart.series.yValues.length);
                                                    //console.log('seriesNameIndex', seriesNameIndex);
